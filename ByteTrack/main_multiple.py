...
class ObjectInstance:
    ...
    number_on_hand: int  # sequence index among current simultaneous objects
    object_time_in_person: int = (
        1  # accumulated frames in hand (increments when detected, decrements when absent)
    )
    last_bbox: Optional[np.ndarray] = None  # xyxy for matching continuity
    active_in_current_frame: bool = True  # set True when detected this frame

    def to_dict(self) -> Dict[str, Any]:
        return {
            "class_objects_names_dict": self.object_class_name,
            "number_on_hand": self.number_on_hand,
            "object_time_in_person": self.object_time_in_person,
        }
...
class TrackerState:
    ...
    def all_objects_list(self) -> List[ObjectInstance]:
        ...
        for class_name in sorted(self.objects_by_class.keys()):
            # keep ordering by class then number_on_hand
            instances = sorted(
                self.objects_by_class[class_name], key=lambda o: o.number_on_hand
            )
...
def compute_wrist_region(
    single_keypoints_xy: np.ndarray,
...
def update_tracker_objects(
    ...
    """Update or create object instance for a tracker.

    Rules:
    - If matches existing (IoU >= threshold) -> increment object_time_in_person
    - Else create new instance with number_on_hand = current count + 1
    - number_on_hand values remain contiguous (reindexed after removals elsewhere)
    """
    ...
    for inst in instances:
        if inst.last_bbox is None:
            continue
        val = iou(np.array(inst.last_bbox), object_box)
        if val > best_iou_val:
            best_iou_val = val
            best_inst = inst
    if best_inst is not None and best_iou_val >= iou_match_threshold:
        best_inst.object_time_in_person += 1
        best_inst.last_bbox = object_box.copy()
        best_inst.active_in_current_frame = True
        return
    # Create new
    new_instance = ObjectInstance(
        object_class_name=object_class_name,
        number_on_hand=len(instances) + 1,
        object_time_in_person=1,
...
def decay_and_cleanup_tracker_objects(tracker_state: TrackerState) -> None:
    """For each non-active instance decrement object_time_in_person.
    Remove instances that reach 0 and reindex number_on_hand for remaining instances per class.
    """
    for class_name, instances in list(tracker_state.objects_by_class.items()):
        # Decrement
        for inst in list(instances):
            if not inst.active_in_current_frame:
                inst.object_time_in_person -= 1
            if inst.object_time_in_person <= 0:
                instances.remove(inst)
        # Reindex number_on_hand
        for idx, inst in enumerate(instances, start=1):
            inst.number_on_hand = idx
        if not instances:
        ...
        for tid, state in tracker_states.items():
            writer.writerow(
                [
                    ...
                    json.dumps(
                        [obj.to_dict() for obj in state.all_objects_list()],
                        ensure_ascii=False,
                    ),
                ]
            )
...
def video_process(
    ...
    with VideoSink(_video_output_path, video_info) as sink:
        ...
        for frame in tqdm(generator, total=video_info.total_frames):
            ...
            # Build person detections safely (handle no detections)
            if (
                detected_person is None
                or len(detected_person) == 0
                or detected_person[0].boxes is None
                or detected_person[0].boxes.xyxy is None
                or detected_person[0].boxes.xyxy.shape[0] == 0
            ):
                detections_person = Detections(
            ...
            detections_person = filter_min_height(detections_person, frame, ratio=0.5)
            ...
            person_boxes_for_tracker = detections2boxes(detections=detections_person)
            tracks = (
                byte_tracker.update(
                    output_results=person_boxes_for_tracker,
                    img_info=frame.shape,
                    img_size=frame.shape,
                )
                if person_boxes_for_tracker.shape[0] > 0
                else []
            )
            ...
            # Build list of indices (instead of boolean mask) to stay robust when counts mismatch
            tracker_id_list = list(detections_person.tracker_id)
            valid_indices = [
                i for i, tid in enumerate(tracker_id_list) if tid is not None
            ]
            ...
            if pose_keypoints_xy is not None:
                # Safety: clamp indices within range
                max_kp = pose_keypoints_xy.shape[0]
                safe_indices = [i for i in valid_indices if i < max_kp]
                if len(safe_indices) != len(valid_indices):
                    # If mismatch occurs, attempt simple fallback: if counts equal after filtering, take first N
                    if len(detections_person) == max_kp:
                        safe_indices = list(range(max_kp))
                try:
                    filtered_keypoints_xy = pose_keypoints_xy[safe_indices]
                    if pose_keypoints_conf is not None:
                        filtered_keypoints_conf = pose_keypoints_conf[safe_indices]
                except Exception:
                    # Final fallback: do not draw keypoints this frame
                    filtered_keypoints_xy = None
                    filtered_keypoints_conf = None
            ...
            for det_index, (_, _, _, tracker_id_value) in enumerate(detections_person):
                ...
                # pose related
                if (
                    filtered_keypoints_xy is not None
                    and det_index < filtered_keypoints_xy.shape[0]
                ):
