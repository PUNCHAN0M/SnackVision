...
class tracklet:
    ...
    def velocity(self):
        if self.rect_num < 2:
            ...
            return (
                self.rect_list[self.rect_num - 1].position
                - self.rect_list[self.rect_num - 2].position
            ) / (
                self.rect_list[self.rect_num - 1].curr_frame
                - self.rect_list[self.rect_num - 2].curr_frame
            )
        else:
            v1 = (
                self.rect_list[self.rect_num - 1].position
                - self.rect_list[self.rect_num - 4].position
            ) / (
                self.rect_list[self.rect_num - 1].curr_frame
                - self.rect_list[self.rect_num - 4].curr_frame
            )
            v2 = (
                self.rect_list[self.rect_num - 2].position
                - self.rect_list[self.rect_num - 5].position
            ) / (
                self.rect_list[self.rect_num - 2].curr_frame
                - self.rect_list[self.rect_num - 5].curr_frame
            )
            v3 = (
                self.rect_list[self.rect_num - 3].position
                - self.rect_list[self.rect_num - 6].position
            ) / (
                self.rect_list[self.rect_num - 3].curr_frame
                - self.rect_list[self.rect_num - 6].curr_frame
            )
            return (v1 + v2 + v3) / 3
...
def cal_simi_track_det(track, det_rect):
    if det_rect.curr_frame <= track.last_frame:
        ...
        pred_rect = track.last_rect.curr_rect + np.append(
...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset):
    ...
    img_list = [
        os.path.join(dataset_path, subset, cur_dataset, "img1", _)
        for _ in img_list
    ...
    for idx in range((int(img_len / 2)), img_len + 1):
        ...
        with torch.no_grad():
            ...
            img1 = (img1.astype(float) / 255.0 - np.array([[RGB_MEAN]])) / np.array(
                [[RGB_STD]]
            )
            img1 = (
                torch.from_numpy(img1).permute(2, 0, 1).view(1, 3, resize_h, resize_w)
            )
            scores, transformed_anchors, last_feat = retinanet(
                img1.cuda().float(), last_feat=last_feat
            )
            # 			if idx > 0:
            if idx > (int(img_len / 2)):
                ...
                matches, unmatched1, unmatched2 = track_det_match(
                    tracklet_all, det_list_all[i], IOU_threshold
                )
                ...
                for j in range(len(unmatched1)):
                    tracklet_all[unmatched1[j]].no_match_frame = (
                        tracklet_all[unmatched1[j]].no_match_frame + 1
                    )
                    if (
                        tracklet_all[unmatched1[j]].no_match_frame
                        >= retention_threshold
    ...
    for i in range((int(img_len / 2)), img_len):
        ...
        for j in range(len(det_list_all[i])):
            ...
            draw_caption(
                ...
                (x1, y1, x2, y2),
                draw_trace_id,
                color=color_list[trace_id % len(color_list)],
            )
            cv2.rectangle(
                img,
                (x1, y1),
...
def run_from_train(model_dir, root_path):
    ...
    for seq_num in [2, 4, 5, 9, 10, 11, 13]:
        run_each_dataset(
            model_dir, retinanet, root_path, "train", "MOT17-{:02d}".format(seq_num)
        )
    for seq_num in [1, 3, 6, 7, 8, 12, 14]:
        run_each_dataset(
            model_dir, retinanet, root_path, "test", "MOT17-{:02d}".format(seq_num)
        )


def main(args=None):
    parser = argparse.ArgumentParser(
        description="Simple script for testing a CTracker network."
    )
    parser.add_argument(
        "--dataset_path",
        default="/dockerdata/home/jeromepeng/data/MOT/MOT17/",
        type=str,
        help="Dataset path, location of the images sequence.",
    )
    parser.add_argument(
        "--model_dir", default="./trained_model/", help="Path to model (.pt) file."
    )
    parser.add_argument(
        "--model_path",
