...
class Tracker(object):
    ...
    def step(self, results_with_low, public_det=None):
        results = [
            item for item in results_with_low if item["score"] >= self.opt.track_thresh
        ]
        ...
        dets = np.array(
            [det["ct"] + det["tracking"] for det in results], float
        )  # N x 2
        track_size = np.array(
            [
                (
        ...
        tracks = np.array([pre_det["ct"] for pre_det in self.tracks], float)  # M x 2
        ...
        if self.opt.dataset == "youtube_vis":
            invalid = (
                ...
                + (box_ious < self.opt.overlap_thresh)
            ) > 0
        else:
            invalid = (
                (dist > track_size.reshape(1, M))
        ...
        for m in matches:
            ...
            if "embedding" in track:
                ...
                self.embedding_bank[self.tracks[m[1]]["tracking_id"] - 1, :] = (
                    self.alpha * track["embedding"]
                    + (1 - self.alpha)
                    * self.embedding_bank[self.tracks[m[1]]["tracking_id"] - 1, :]
                )
        ...
        if self.opt.public_det and len(unmatched_dets) > 0:
            ...
            matched_dets = [
                d for d in range(dets.shape[0]) if not (d in unmatched_dets)
            ]
            ...
            for i in unmatched_dets:
                ...
                if track["score"] > self.opt.new_thresh:
                    if "embedding" in track:
                        max_id, max_cos = self.get_similarity(
                            track["embedding"], False, track["class"]
                        )
                        if (
                            max_cos >= 0.3
                            and self.tracklet_ages[max_id - 1] < self.opt.window_size
                        ):
                            track["tracking_id"] = max_id
                            track["age"] = 1
                            track["active"] = 1
                            self.embedding_bank[track["tracking_id"] - 1, :] = (
                                self.alpha * track["embedding"]
                                + (1 - self.alpha)
                                * self.embedding_bank[track["tracking_id"] - 1, :]
                            )
                            ...
                            track["age"] = 1
                            track["active"] = 1
                            self.embedding_bank[self.id_count - 1, :] = track[
                                "embedding"
                            ]
        ...
        self_tracks_second = [
            self.tracks[i] for i in unmatched_tracks if self.tracks[i]["active"] > 0
        ]
        ...
        if N > 0 and M > 0:
            ...
            track_boxes_second = np.array(
                [
                    ...
                    for track in self_tracks_second
                ],
                float,
            )  # M x 4
            det_boxes_second = np.array(
                [
                    [item["bbox"][0], item["bbox"][1], item["bbox"][2], item["bbox"][3]]
                    for item in results_second
                ],
                float,
            )  # N x 4
            box_ious_second = self.bbox_overlaps_py(
                det_boxes_second, track_boxes_second
            )

            dets = np.array(
                [det["ct"] + det["tracking"] for det in results_second], float
            )  # N x 2
            track_size = np.array(
                [
                    (
            ...
            track_cat = np.array(
                [track["class"] for track in self_tracks_second], np.int32
            )  # M
            item_size = np.array(
                [
                    (
            ...
            item_cat = np.array(
                [item["class"] for item in results_second], np.int32
            )  # N
            tracks_second = np.array(
                [pre_det["ct"] for pre_det in self_tracks_second], float
            )  # M x 2
            dist = (
                (tracks_second.reshape(1, -1, 2) - dets.reshape(-1, 1, 2)) ** 2
            ).sum(
                axis=2
            )  # N x M
            ...
            unmatched_tracks_second = [
                d
                for d in range(tracks_second.shape[0])
            ...
            for m in matches_second:
                ...
                if "embedding" in track:
                    self.alive.append(track["tracking_id"])
                    self.embedding_bank[
                        self_tracks_second[m[1]]["tracking_id"] - 1, :
                    ] = (
                        self.alpha * track["embedding"]
                        + (1 - self.alpha)
                        * self.embedding_bank[
                    ...
                    self.cat_bank[self_tracks_second[m[1]]["tracking_id"] - 1] = track[
                        "class"
                    ]
                ret.append(track)
