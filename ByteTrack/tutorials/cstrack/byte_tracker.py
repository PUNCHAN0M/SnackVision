...
from core.mot.general import (
    non_max_suppression_and_inds,
    non_max_suppression_jde,
    non_max_suppression,
    scale_coords,
)
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score):
        ...
        # wait activate
        self._tlwh = np.asarray(tlwh, dtype=float64)
...
class BYTETracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        self.model = Model(opt.cfg or ckpt["model"].yaml, ch=3, nc=1).to(
            opt.device
        )  # create
    ...
    def update(self, im_blob, img0, seq_num, save_dir):
        ...
        detections = []
        if len(pred) > 0:
            dets, x_inds, y_inds = non_max_suppression_and_inds(
                pred[:, :6].unsqueeze(0), 0.1, self.opt.nms_thres, method="cluster_diou"
            )
            dets = dets.numpy()
            if len(dets) != 0:
                ...
                detections = [
                    STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), tlbrs[4])
                    for tlbrs in dets[:, :5]
                ]
        ...
        if self.opt.vis_state == 1 and self.frame_id % 20 == 0:
            if len(dets) != 0:
                ...
                (
                    ...
                    cost_matrix_det,
                    cost_matrix_track,
                ) = matching.vis_id_feature_A_distance(strack_pool, detections)
            vis_feature(
                self.frame_id,
                seq_num,
                img0,
...
def vis_feature(
    ...
    if len(cost_matrix_track) != 0 and len(cost_matrix_track[0]) != 0:
        ...
        cost_matrix_track = np.round(
            (cost_matrix_track - min_f) / (max_f - min_f) * 255
        )
        d_F_M = []
