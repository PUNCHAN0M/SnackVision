...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, dataset2transform):
        ...
        self.item_num = (
            len(self.img_files) - (self.num_frames_per_batch - 1) * self.sample_interval
        )
        ...
        if self.sampler_steps is not None and len(self.sampler_steps) > 0:
            ...
            self.item_num = (
                len(self.img_files) - (self.lengths[-1] - 1) * self.sample_interval
            )
            self.period_idx = 0
    ...
    def _pre_single_frame(self, idx: int):
        ...
        obj_idx_offset = (
            self.video_dict[video_name] * 1000000
        )  # 1000000 unique ids is enough for a video.
        if "crowdhuman" in img_path:
        ...
        targets["boxes"] = torch.as_tensor(targets["boxes"], dtype=torch.float).reshape(
            -1, 4
        )
    ...
    def __getitem__(self, idx):
        ...
        images, targets = self.pre_continuous_frames(
...
def make_transforms_for_mot17(image_set, args=None):
    ...
    normalize = T.MotCompose(
        [T.MotToTensor(), T.MotNormalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]
    )
