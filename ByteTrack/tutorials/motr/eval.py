...
class Detector(object):
    def __init__(self, args, model=None, seq_num=2):
        ...
        img_list = [
            os.path.join(self.args.mot_path, self.seq_num, "img1", _)
            for _ in img_list
    ...
    def visualize_img_with_bbox(
        ...
        if dt_instances.has("scores"):
            img_show = draw_bboxes(
                img,
                np.concatenate(
                    [dt_instances.boxes, dt_instances.scores.reshape(-1, 1)], axis=-1
                ),
    ...
    def detect(self, prob_threshold=0.2, area_threshold=100, vis=False):
        ...
        for i in tqdm(range((int(self.img_len / 2)), self.img_len)):
            ...
            if vis:
                # for visual
                cur_vis_img_path = os.path.join(
                    self.save_path, "frame_{:0>8d}.jpg".format(i)
                )
                gt_boxes = None
            ...
            self.write_results(
                txt_path=os.path.join(self.predict_path, "gt.txt"),
                frame_id=(i + 1),
