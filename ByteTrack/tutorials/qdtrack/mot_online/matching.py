...
def embedding_distance2(tracks, detections, metric="cosine"):
    ...
    cost_matrix2 = np.maximum(
        0.0, cdist(track_features, det_features, metric)
    )  # Nomalized features
    track_features = np.asarray(
        [track.features[len(track.features) - 1] for track in tracks], dtype=float
    )
    cost_matrix3 = np.maximum(
        0.0, cdist(track_features, det_features, metric)
    )  # Nomalized features
    for row in range(len(cost_matrix)):
        cost_matrix[row] = (
            cost_matrix[row] + cost_matrix2[row] + cost_matrix3[row]
        ) / 3
    return cost_matrix
...
def vis_id_feature_A_distance(tracks, detections, metric="cosine"):
    ...
    if leg1 != 0:
        cost_matrix_track = np.maximum(
            0.0, cdist(track_features, track_features, metric)
        )
    if cost_matrix.size == 0:
        return (
            track_features,
